...
**Ported and working:** sign-in (magic link), Home (Team P1s + Follow-Ups),
Daily Priorities (ticket cards, flagging to P1, due dates, status, per-day
schedule), the KPI tab (department tabs, seniority sorting, per-week notes,
name search, real privacy — admin sees everyone, a person sees only their
own row once explicitly shared, everyone else doesn't see the tab exists),
and real-time updates across browsers.
...
5. Authentication → Providers → make sure Email is on, "Confirm email" can
   stay off if you want the magic link to sign people in immediately.
6. Authentication → URL Configuration → add your eventual Vercel URL (and
...
account). Then in the SQL Editor:
```sql
...
### 3. GitHub
Push this whole folder to a new repo.
